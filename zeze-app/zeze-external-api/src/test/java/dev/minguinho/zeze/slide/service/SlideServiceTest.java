...
import static dev.minguinho.zeze.slide.model.Slide.*;
...
class SlideServiceTest {
    ...
    void createSlide() {
        ...
        SlideRequestDto slideRequestDto = new SlideRequestDto(title, subtitle, author, presentedAt, content,
            accessLevel);
        given(slideRepository.save(any(Slide.class))).willReturn(SlideConvertor.toEntity(slideRequestDto));
    ...
    void retrieveSlides() {
        ...
        List<Slide> slides = Arrays.asList(
            ...
            new Slide(secondTitle, secondSubtitle, secondAuthor, secondPresentedAt, secondContent, AccessLevel.PRIVATE,
                1L));
        given(slideRepository.findAllByAccessLevelAndDeletedAtIsNullOrderByUpdatedAtDesc(eq(AccessLevel.PUBLIC),
            any(Pageable.class)))
            .willReturn(page);
